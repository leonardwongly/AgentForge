...
The preview uses last-match-wins CODEOWNERS precedence, normalizes `@org/team` to `org/team`, and reports unsupported negated patterns, unsupported bracket patterns, missing `@` owner prefixes, and malformed owners as diagnostics instead of silently treating them as owner routes.
