...
- In production, the API accepts trusted proxy identity headers (`x-agentforge-authenticated-actor`, `x-agentforge-authenticated-role`, and `x-agentforge-authenticated-organization`) only when `AGENTFORGE_API_TRUST_PROXY_HEADERS=true` and `AGENTFORGE_AUTH_PROXY_STRIPS_HEADERS=true`. Raw local actor headers (`x-agentforge-actor`, `x-agentforge-role`, and `x-agentforge-organization`) are rejected in production and require `AGENTFORGE_API_ALLOW_LOCAL_ACTOR_HEADERS=true` outside tests.
