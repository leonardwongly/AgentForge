...
5. Logins listed in `AGENTFORGE_GITHUB_ADMIN_LOGINS` receive `platform_admin`; other GitHub users receive `developer`.
...
4. AgentForge stores the installation as `pending_approval`.
5. A platform admin reviews the account login/type and approves it.
6. AgentForge links the installation to the current AgentForge organization and syncs repositories from stored installation events.
